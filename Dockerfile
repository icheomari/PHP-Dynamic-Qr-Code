...
# Install PHP extensions (build-time; later runtime extensions are installed in the final stage)
...
# Clone php-qrcode repository and copy its needed files
WORKDIR /opt
...
# and run Composer update to install PHP dependencies.
WORKDIR /var/www/html
COPY ./src ./ 
RUN composer update --no-dev --optimize-autoloader

# Ensure proper permissions for the app files
...
# Install runtime dependencies and required PHP extensions.
RUN apt-get update -q && \
    apt-get install -qq -y \
      libzip-dev \
      libjpeg62-turbo-dev \
...
# (If an extension was installed during build, you can often just enable it here.)
...
# Expose port 80 and use PHP's built-in server for runtime.
EXPOSE 80
